...
/*
   ...
   Version 3.8
    -- Try and fix bug whereby Wifi client (access point) mode was failing.
   Version 3.8
...
#define VERBOSE           true    // Can also be toggled at run-time (hit 'v' on serial monitor)
...
#define ESP32_RESTORE_DEFAULTS_PIN  34  // ESP32 WiFi: ground to restore vars defaults at wifi startup (CONFLICT with SPI SCK)
#define GPS_RX_PIN                  33  // RX-from-GPS pin.  GPS's TX pin (white) gets connected here
#define GPS_TX_PIN                  25  // TX-to-GPS pin. Possibly wired to the GPS RX pin, but we never send anything (CONFLICT with I2C SCL)
...
static bool wifi_begin_client_mode()
{
  ...
  unsigned long timeout = get_timeout(5 * 60 * 1000);   // 5-minute failsafe timeout
  WiFi.begin(ssid, passkey);
  while (WiFi.status() != WL_CONNECTED) {
    service_non_wifi_devices();
    if (time_after(millis(), timeout)) {
      Serial.println("WiFi client mode aborted");
      wifi_client_mode_override = true;
      return false;
    }
  }
...
static void set_esp32_wifi (bool want_off)
{
  ...
  if (want_off) {
    ...
    if (wifi_client_mode) {
      ...
      Serial.println("esp32_wifi client mode");
    }
    if (!wifi_client_mode) {
      Serial.println("esp32_wifi server mode");
